...
K <- 17 #total number of exposures
...
K.m1 <- 8 #number of exposures in model 1
K.m2 <- 17 #number of exposures in model 2
x.false.names.m1 <- c("MEP", "MOH", "MHH", "CNP")
x.inc.m1 <- x.names %in% c(x.true.names, x.false.names.m1) #included in model 1
x.inc.m2 <- rep(TRUE, K.m2) #included exposures in model 2
...
iter.bkmr <- 10000 #including burn-in
iter.burnin.bart <- 4000
iter.bart <- 6000 #including burn-in
iter.burnin.star <- 8000
iter.star <- 10000 #including burn-in
